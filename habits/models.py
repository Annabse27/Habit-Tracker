...
class Habit(models.Model):
    ...
    is_public = models.BooleanField(
        default=False, help_text="Привычка публичная?")
    next_reminder = models.DateTimeField(
        null=True, blank=True, help_text="Дата и время следующего напоминания")
    ...
    def clean(self):
        # Проверка, что у приятной привычки нет связанной привычки или
        # вознаграждения
        ...
        # Проверка, что у привычки не указаны одновременно связанная привычка и
        # вознаграждение
        if self.linked_habit and self.reward:
            raise ValidationError(
                'Нельзя указать одновременно связанную привычку и вознаграждение.')

        # Проверка на продолжительность выполнения привычки (не больше 120
        # секунд)
        ...
        if self.frequency < 1 or self.frequency > 7:
            raise ValidationError(
                'Привычка должна повторяться не реже 1 раза в 7 дней.')
